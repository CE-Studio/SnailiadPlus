...
public class GravTurtle2 : Enemy
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < dirs.Length; i++)
        {
            for (int j = 0; j < states.Length; j++)
                anim.Add("Enemy_gravturtle2_" + dirs[i] + "_" + states[j]);
        }
    }
    ...
    private void Update()
    {
        ...
        if (PlayState.OnScreen(transform.position, box))
        {
            string animPrefix = "Enemy_gravturtle2_" + gravityDir.ToString() + "_";
            switch (animState)
            {
