...
public class Kitty : Enemy
{
    ...
    private const float WEAPON_TIMEOUT = 0.12f;
    ...
    void Awake()
    {
        ...
        anim.Add("Enemy_kitty1_fall");
        anim.Add("Enemy_kitty1_shoot");
    ...
    void FixedUpdate()
    {
        ...
        if (PlayState.OnScreen(transform.position, box))
        {
            ...
            if (isAttacking && actionTimeout < 0 && shotCount >= 0)
            {
                ...
                PlayState.ShootEnemyBullet(this, transform.position, EnemyBullet.BulletType.pea, facingRight ? Mathf.PI - fireAngle : fireAngle, WEAPON_SPEED);
                anim.Play("Enemy_kitty1_shoot");
            }
            if (shotCount <= 0)
            {
                if (actionTimeout <= 0)
        ...
        if (velocity.y < 0 && anim.currentAnimName == "Enemy_kitty1_jump")
            PlayAnim("Enemy_kitty1_fall");
    }
