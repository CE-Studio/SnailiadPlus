...
public class RushSpaceBox : Boss
{
    ...
    private const int DAMAGE_TAKEN_FOR_SHIELD = 250;
    private const int SHIELD_PERIOD = 3;
    private const float MODE_TIMEOUT = 0.6f;
    private const float SPAWN_TIMEOUT = 2.5f;
    private const float STARTING_SPEED = 1.4f;
    ...
    public List<RushSpaceBoxShield> shields = new();
    public List<RushSpaceBoxBabybox> babyboxes = new();
    ...
    private int SHOT_COUNT = 7;
    private float SHOT_TIMEOUT = 0.4f;
    ...
    private void Awake()
    {
        ...
        if (PlayState.IsBossAlive(2))
        {
                ...
                foreach (string animType in bossAnimTypes)
                {
                    ...
                    if (formattedAnimType.Contains("$"))
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            ...
                            anim.Add("RushBoss_spaceBox_" + furtherFormattedAnimType);
                        }
                    }
                    else
                        anim.Add("RushBoss_spaceBox_" + formattedAnimType);
                }
            }
            ...
            while (childCount < SHIELD_SLOTS)
            {
                ...
                RushSpaceBoxShield shieldComponent = newShield.GetComponent<RushSpaceBoxShield>();
            ...
            PlayState.globalFunctions.CreateLightMask(23, transform);
        }
        else
            Destroy(gameObject);
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (stopMoving)
        {
            anim.Play("RushBoss_spaceBox_hit" + attackMode.ToString() + "_" + dirString);
            Stomp();
        }
        ...
        if (health <= maxHealth * 0.3f && attackMode < 1)
        {
            speed += 0.5f;
            attackMode = 1;
            foreach (RushSpaceBoxShield shield in shields)
                shield.TurnBlue();
            foreach (RushSpaceBoxBabybox babybox in babyboxes)
                babybox.TurnBlue();
            SHOT_COUNT = 6;
        ...
        if (!anim.isPlaying)
            anim.Play("RushBoss_spaceBox_idle" + attackMode.ToString());
    }
    ...
    private void UpdateShieldPositions()
    {
        ...
        {
            timeValue = (elapsed / SHIELD_PERIOD % 1f * SHIELD_SLOTS + 17f * shieldID % MAX_ACTIVE_SHIELDS) % SHIELD_SLOTS;
    ...
    private void Charge(BossMode dir)
    {
        ...
        anim.Play("RushBoss_spaceBox_charge" + attackMode.ToString() + "_" + dirString);
    }
    ...
    {
        anim.Play("RushBoss_spaceBox_waitSpawn" + attackMode.ToString());
        ...
        if (babyboxes.Count < MAX_BOXES)
        {
            if (attackMode == 0)
            {
                ...
                box1.GetComponent<RushSpaceBoxBabybox>().Activate(this, true);
                GameObject box2 = Instantiate(babyboxObj, transform.position, Quaternion.identity, parentRoom);
                box2.GetComponent<RushSpaceBoxBabybox>().Activate(this, false);
            }
            else
            {
                ...
                box1.GetComponent<RushSpaceBoxBabybox>().Activate(this, false);
                ...
                box2.GetComponent<RushSpaceBoxBabybox>().Activate(this, true);
                ...
                box3.GetComponent<RushSpaceBoxBabybox>().Activate(this, true);
                ...
                box4.GetComponent<RushSpaceBoxBabybox>().Activate(this, false);
            }
        }
        mode = BossMode.Wait;
