...
public class RushMoonSnail : Boss
{
    ...
    private readonly float[] weaponTimeouts = new float[] { 0.05f, 0.15f, 0.1025f };
    private readonly float[] weaponSpeeds = new float[] { 0.925f, 16.875f, 0.125f };
    ...
    private void Awake()
    {
        ...
        {
            SpawnBoss(12500, 0, 30, true, 0, 3, true);
            if (debugSkipToGiga)
            ...
            else
            {
                ...
                for (int i = 0; i < SHADOW_BALL_COUNT; i++)
                {
                    ...
                    ballAnim.Add("RushBoss_moonSnail_shadowBall1");
                    ballAnim.Add("RushBoss_moonSnail_shadowBall2");
                    ballAnim.Play("RushBoss_moonSnail_shadowBall1");
                ...
                for (int i = 0; i < 2; i++)
                {
                    for (int j = 0; j < states.Length; j++)
                    {
                        for (int k = 0; k < directions.Length; k++)
                            anim.Add("RushBoss_moonSnail_" + states[j] + (i + 1).ToString() + "_" + directions[k]);
                    }
                }
                animData = PlayState.GetAnim("RushBoss_moonSnail_data").frames;
    ...
    private void UpdateAITeleport()
    {
        ...
        if (!modeInitialized)
        {
            ...
            for (int i = 0; i < shadowBalls.Count; i++)
            {
                ...
                if (animData[5] == 1)
                    shadowBallAnims[i].Play("RushBoss_moonSnail_shadowBall" + (attackPhase + 1).ToString());
            }
            lightMask.SetSize(-1);
        }
    ...
    private void FixedUpdate()
    {
        ...
        PlayState.activeRushData.sunTime += Time.fixedDeltaTime;
        ...
        string expectedAnim = "RushBoss_moonSnail";
        ...
        if (health <= maxHealth * 0.4f && attackPhase < 1)
        {
            speed += 0.2f;
            attackPhase = 1;
    ...
    public override void Kill()
    {
        ...
        Instantiate(Resources.Load<GameObject>("Objects/Enemies/Bosses/Giga Sun Snail"), gigaSpawnPoint.pos, Quaternion.identity, transform.parent);
        Destroy(gameObject);
    }
}
