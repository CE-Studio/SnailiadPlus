...
public class RushGigaSnail : Boss
{
    private const float GRAV_JUMP_TIMEOUT = 0.1f;
    private const float START_ATTACK_TIME = 0.4f;
    ...
    private float WAVE_TIMEOUT = 1.0f;
    private float WAVE_SPEED = 1.25f;
    ...
    public void Awake()
    {
        ...
        animData = PlayState.GetAnim("RushBoss_gigaSnail_data").frames;
        if (animData[(int)AnimData.FadeInOnSpawn] == 1)
            sprite.color = new Color32(255, 255, 255, 0);
        for (int i = 1; i <= 2; i++)
        {
            anim.Add("RushBoss_gigaSnail_idle" + i.ToString());
            anim.Add("RushBoss_gigaSnail_strafe" + i.ToString());
            for (int j = 0; j < PlayState.DIRS_COMPASS.Length; j++)
            {
                anim.Add("RushBoss_gigaSnail_smash" + i.ToString() + "_" + PlayState.DIRS_COMPASS[j]);
                anim.Add("RushBoss_gigaSnail_smash" + i.ToString() + "_collide_" + PlayState.DIRS_COMPASS[j]);
            }
            anim.Add("RushBoss_gigaSnail_sleep" + i.ToString() + "_shelled");
            anim.Add("RushBoss_gigaSnail_sleep" + i.ToString() + "_land");
            for (int j = 0; j < PlayState.DIRS_SURFACE.Length; j++)
            {
                for (int k = 0; k < 2; k++)
                {
                    ...
                    anim.Add("RushBoss_gigaSnail_sleep" + i.ToString() + "_unshelled_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_unshell_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_idle_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_turn_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_jump_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_turnAir_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_flip_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_land_" + thisDir + "_" + dirMod);
                    anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_shell_" + thisDir + "_" + dirMod);
                    if (i == 1)
                    {
                        anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_turnRed_" + thisDir + "_" + dirMod);
                        anim.Add("RushBoss_gigaSnail_stomp" + i.ToString() + "_turnRedAir_" + thisDir + "_" + dirMod);
                    }
                }
            }
        }
        anim.Add("RushBoss_gigaSnail_intro");
        anim.Play("RushBoss_gigaSnail_intro");
    ...
    private void UpdateBackground(string newState)
    {
        ...
        switch (newState)
        {
            ...
            case "fadeOut":
                SetBackgroundState(true, false);
                if (bgAnimData[1] == 1)
                    StartCoroutine(FadeBackground(false, 2f, false));
    ...
    private void UpdateAIStomp()
    {
        ...
        if ((attackPhase == 0 && modeElapsed > START_ATTACK_TIME * 3f) || (attackPhase == 1 && modeElapsed > START_ATTACK_TIME * 3.5f))
            ShootWave();
        if (modeTimeout <= 0)
        {
    ...
    private void UpdateAIStrafe()
    {
        ...
        if (modeElapsed > START_ATTACK_TIME && !aimed && Vector2.Distance(transform.position, moveTarget) < 0.625f)
        {
            ...
            strafeThetaAccel = GetDecision() > 0.5f ? -strafeThetaVel * 1.3f : 0;
            aimed = true;
            if (PlayState.currentProfile.difficulty == 2)
                strafeThetaVel *= 1.6f;
            if (anim.currentAnimName != "RushBoss_gigaSnail_strafe" + (attackPhase + 1).ToString() && animData[(int)AnimData.UpdateAnimOnPhaseChange] == 1)
                PlayAnim(AnimTypes.strafe);
        }
        strafeThetaVel = Mathf.Clamp(strafeThetaVel, -Mathf.PI / 5, Mathf.PI / 5);
    ...
    private void FixedUpdate()
    {
        ...
        PlayState.activeRushData.gigaTime += Time.fixedDeltaTime;
        ...
        if ((lastAnimState == "shell" || (mode == BossMode.Sleep && !grounded)) && mode != BossMode.Intro)
        {
            ...
            if (trailTimeout <= 0)
            {
                ...
                PlayState.RequestParticle(transform.position, "rushGigaTrail", new float[] { finalSpriteID, sprite.flipX ? 1 : 0, sprite.flipY ? 1 : 0 });
            }
        }
        else
            trailTimeout = 0;
    ...
    {
        string animName = "RushBoss_gigaSnail_";
        ...
        if (!anim.isPlaying)
            animName = "RushBoss_gigaSnail_idle" + (attackPhase + 1).ToString();
    ...
    private void AimStrafe()
    {
        ...
        strafeCount = Mathf.Clamp(Mathf.FloorToInt(2.3f + 5f * (maxHealth - health) / maxHealth), 4, 7);
    ...
    private void PickSmashDir(bool guaranteeTargetPlayer = false)
    {
        ...
        if (GetDecision() > 0.15f || guaranteeTargetPlayer)
    ...
    public override void Kill()
    {
        ...
        PlayState.globalFunctions.RequestQueuedExplosion(transform.position, 2.7f, 1, true);
        PlayState.globalFunctions.ScreenShake(new List<float> { 0.075f, 0.075f, 0 }, new List<float> { 1.7f, 1.2f });
