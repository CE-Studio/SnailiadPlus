...
public class RushStompyEye : Enemy
{
    private RushStompy boss;
    ...
    private const float CLUSTER_TIMEOUT = 4.6f;
    private const float SHOT_TIMEOUT = 0.3f;
    ...
    private void Awake()
    {
        ...
        boss = transform.parent.GetComponent<RushStompy>();
    ...
    private void Shoot(float angle)
    {
        ...
        PlayState.ShootEnemyBullet(this, pupil.transform.position, EnemyBullet.BulletType.donutLinear, new float[] { 5.625f, angleX, angleY });
    }
    ...
    private void Update()
    {
        ...
        if (isOpen)
        {
            ...
            if (blinkTimeout < 0)
            {
                ...
                eyelidAnim.Play("RushBoss_stompy_eyelid" + (isLeft ? "L" : "R") + boss.attackMode.ToString() + "_blink");
            }
            if (willClose)
            {
                ...
                if (closeTimeout < 0)
                {
                    willClose = false;
                    isOpen = false;
                    eyelidAnim.Play("RushBoss_stompy_eyelid" + (isLeft ? "L" : "R") + boss.attackMode.ToString() + "_close");
                    openTimeout = 0.8f;
                }
            }
        }
        else
        {
            ...
            {
                isOpen = true;
                eyelidAnim.Play("RushBoss_stompy_eyelid" + (isLeft ? "L" : "R") + boss.attackMode.ToString() + "_open");
            }
        }
    }
