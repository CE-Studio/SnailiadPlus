...
public class Tallfish1 : Enemy
{
    private const float MOVE_TIMEOUT = 2.3f;
    private const float SHOT_TIMEOUT = 0.25f;
    private const int SHOT_COUNT = 3;
    ...
    private void Awake()
    {
        ...
        anim.Add("Enemy_tallfish_green_attackForward");
        anim.Add("Enemy_tallfish_green_attackTurnaround");
        anim.Play("Enemy_tallfish_green_idle");
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (PlayState.OnScreen(transform.position, col))
        {
            ...
            if (moveTimeout <= 0)
            {
                ...
                {
                    if (facingLeft)
                        anim.Play("Enemy_tallfish_green_attackForward");
                    else
                        anim.Play("Enemy_tallfish_green_attackTurnaround");
                    facingLeft = true;
                    sprite.flipX = true;
