...
const TELEPORT_TIME:float = 0.95
...
const RUN_SPEED:float = 410.0
const MAX_SPEED:float = 600.0
const GRAVITY:float = 1200.0
const WEAPON_COOLDOWNS:Array[float] = [ 0.1, 0.3, 0.205 ]
const WEAPON_SPEED:Array[float] = [ 280.0, 330.0, 100.0 ]
...
const DEATH_TIME:float = 2.5
const DEATH_MOVE_TIME:float = 2.0
const DEATH_FADE_START:float = 1.75
const DEATH_FADE_END:float = 2.5
const DEATH_FADE_COLOR:Color = Color("58f89c")
...
func _ready() -> void:
	...
	my_type = EnemyTypes.MOONSNAIL_RUSH
	super.spawn()
...
func kill() -> void:
	...
	if not in_death_anim:
		...
		var giga:GigasnailRush = load("uid://b4c8tmx5pkyqw").instantiate()
...
func _attack(delta:float) -> void:
	...
	match current_weapon:
		...
		2:
			bullets.append(_shoot(LIGHT_WAVE, aim_vector, WEAPON_SPEED[2]))
