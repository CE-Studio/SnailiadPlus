...
const GRAV_JUMP_TIMEOUT:float = 0.1
const START_ATTACK_TIME:float = 0.4
const JUMP_POWER:float = 360.0
const JUMP_TIMEOUT:float = 0.8
const GRAVITY:float = 900.0
const WALK_SPEED:float = 200.0
const WAVE_SPEED:float = 20.0
const WAVE_TIMEOUT:float = 1.0
...
const DECISION_TABLE:Array[float] = [
	0.1640168826, 0.3892556902, 0.0336081053, 0.2246864975, 0.5434009453, 0.4227320437,
	0.1017472328, 0.2041907897, 0.9950191347, 0.3634705228, 0.0779175897, 0.384822732,
	0.3284047846, 0.0951552057, 0.1941055446, 0.496359046,  0.2428007567, 0.8280672868,
	0.852732986,  0.6928913176, 0.2023843678, 0.7280045905, 0.4311591744, 0.796788024,
	0.41191487,   0.7108575032, 0.1134556829, 0.6883870615, 0.8149317527, 0.8392490375,
	0.3647662453, 0.3487805783, 0.7900575239, 0.1670561498, 0.9810836953, 0.0097847681,
	0.2244645569, 0.0842442402, 0.3263779227, 0.1481701068, 0.6538572663, 0.2544128409,
	0.1991950422, 0.541057099,  0.574700257,  0.5926224371, 0.310134571,  0.6104650203,
	0.3545506087, 0.2313309166, 0.3070387696, 0.0790505658, 0.9804949607, 0.7704714904,
	0.7152660213, 0.8215058975, 0.9426850446, 0.7483973576, 0.7602092802, 0.881605898,
	0.5136580468, 0.0190696615, 0.28759162,   0.1565554394, 0.3664312259, 0.2586407176,
	0.3185483313, 0.9837348993, 0.3330417452, 0.2801789805, 0.3288621592, 0.0230039287,
	0.303914672,  0.7212895333, 0.6296904139, 0.8659332532, 0.1715852607, 0.3900271956,
	0.2824020982, 0.1624092775, 0.7599701669, 0.6952292831, 0.2161165745, 0.9005386635,
	0.3707154895, 0.6392742953, 0.452149187,  0.5595775233, 0.686286675,  0.7266258821,
...
var allow_anim_speedup:bool = false
...
func _ready() -> void:
	my_type = EnemyTypes.GIGASNAIL_RUSH
	super.spawn()

	if display_mode:
		sprite.play("p0_stomp_idle_floor_left" if randf() < 0.5 else "p0_stomp_idle_floor_right")
		return
	_play_anim("shell", "shell")
	...
	sfx_spawn.play()
	boss_environment = load("uid://dbb85h70js6kh").instantiate()
	GameCore.instance.current_room.layer_ground.add_child(boss_environment)
	boss_environment.connect_giga(self)

	if sprite.meta_info.size() >= 1 and sprite.meta_info[0] is bool:
		update_if_shell = sprite.meta_info[0]
	if sprite.meta_info.size() >= 2 and sprite.meta_info[1] is bool:
		allow_anim_speedup = sprite.meta_info[1]
		...
		match stomp_gravity:
			Statics.DirsSurface.FLOOR:
				...
				if body.is_on_wall():
					...
					position.x += 0.5 * sign(body.velocity.x)
			Statics.DirsSurface.LWALL:
				body.velocity.x -= GRAVITY * delta
				if body.is_on_wall():
					body.velocity.y = -last_vel.y
					position.y += 0.5 * sign(body.velocity.y)
			...
			Statics.DirsSurface.CEILING:
				...
				if body.is_on_wall():
					body.velocity.x = -last_vel.x
					position.x += 0.5 * sign(body.velocity.x)
		if body.is_on_floor() and mode_elapsed > 0.0:
			_stomp()
	#endregion
	#region Move smash
	if mode == BossMode.SMASH:
		body.velocity += smash_dir * SMASH_SPEED * boss_speed * delta
		if body.move_and_slide():
			if body.is_on_floor():
				...
				position.y += 0.5
			elif body.get_last_slide_collision().get_position().x < position.x:
...
func _pick_smash_target(force_target_player:bool) -> void:
	...
	if _get_decision() > 0.15 or force_target_player:
		angle = atan2(
...
func _spawn_afterimage() -> void:
	...
	match mode:
		BossMode.STOMP: anim = "aftimg_stomp"
		BossMode.SMASH: anim = "aftimg_smash"
		BossMode.STRAFE: anim = "aftimg_strafe"
		BossMode.SLEEP: anim = "aftimg_sleep"
	sprite.call_deferred("create_afterimage", 0.8, 0.0, 0.75, z_index - 1, anim)
...
func _update_stomp(delta:float) -> void:
			...
			if waiting_to_jump and jump_timeout <= 0.0:
				...
				match stomp_gravity:
					Statics.DirsSurface.FLOOR:
						...
						body.velocity.x = -WALK_SPEED if facing_left else WALK_SPEED
					Statics.DirsSurface.LWALL:
						body.velocity.x = JUMP_POWER
						body.velocity.y = -WALK_SPEED if facing_left else WALK_SPEED
					Statics.DirsSurface.RWALL:
						body.velocity.x = -JUMP_POWER
						body.velocity.y = WALK_SPEED if facing_left else -WALK_SPEED
					Statics.DirsSurface.CEILING:
	...
	if ((phase == 0 and mode_elapsed > START_ATTACK_TIME * 3.0)
	or (phase > 0 and mode_elapsed > START_ATTACK_TIME * 3.5)):
		_shoot_wave()
	if mode_timeout <= 0.0:
...
func _update_strafe(delta:float) -> void:
	...
	and position.distance_to(target) < STRAFE_TARGET_PROX):
		...
		strafe_spoke_count = clampi(strafe_spoke_count, 4, 7)
		...
		if _get_decision() > 0.5:
			...
			strafe_theta_vel = -PI / 8.0
		if _get_decision() > 0.5:
			strafe_theta_accel = -strafe_theta_vel * 1.3
		aimed = true
		if Statics.current_profile["difficulty"] == 2:
			strafe_theta_vel *= 1.6
	strafe_theta_vel = clampf(strafe_theta_vel, -PI * 0.2, PI * 0.2)
...
func _face_player(play_anim:bool, _unshell:bool = false) -> void:
