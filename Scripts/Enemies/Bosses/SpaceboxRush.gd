...
const SHIELD_DAMAGE_THRESHOLD:int = 250
const SHIELD_PERIOD:int = 3
...
const SHOT_TIMEOUTS:Array[float] = [0.4, 0.2]
...
const SHIELD:PackedScene = preload("uid://di75wbodi1yul")
const BABYBOX:PackedScene = preload("uid://bqn60v4ab6tvn")
...
func _ready() -> void:
	...
	my_type = EnemyTypes.SPACEBOX_RUSH
	super.spawn()
...
func update_shield_positions() -> void:
	...
	for i in range(shields.size()):
		cycle_point = fmod(fmod(lifetime / SHIELD_PERIOD, 1.0) * MAX_SHIELD_SLOTS + fmod(17.0 * i, MAX_SHIELDS_ACTIVE), MAX_SHIELD_SLOTS)
