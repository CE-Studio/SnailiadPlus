...
class_name SpiderTough
extends Enemy


#region Variables
const SPEED:float = 85.0
const MOVE_TIMEOUT:float = 0.3
...
func _ready() -> void:
	my_type = EnemyTypes.SPIDER_TOUGH
	hitbox = $"Area2D"
	sprite = $"JsonSprite2D"
	vis = $"VisibleOnScreenNotifier2D"
	super.spawn()
	
	SpiderCommon.global_move_index = (SpiderCommon.global_move_index + 1) % WAIT_TABLE.size()
	move_index = SpiderCommon.global_move_index
