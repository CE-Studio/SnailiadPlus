...
func _ready() -> void:
	...
	name_text.set_snaily_text_raw(GlobalText.get_player_name(this_char as Player.Players, true))
...
func _init_item_slots() -> void:
	...
	for i in ITEMS_WEAPON:
		...
		total += count
	if total == 0:
		header_weapon.set_snaily_text_raw(tr(&"?????"))
	...
	for i in ITEMS_SHELL:
		...
		total += count
	if total == 0:
		header_shell.set_snaily_text_raw(tr(&"?????"))
	...
	for i in ITEMS_ABILITY:
		...
		total += count
	if total == 0:
		header_ability.set_snaily_text_raw(tr(&"?????"))
...
func _test_for_move_selection() -> void:
	...
	match move_mode:
		MoveMode.LIST:
			...
			select_text.set_snaily_text_raw(&"Scroll selection - bind__UP bind__DOWN")
		MoveMode.NAME:
		...
		MoveMode.MAP:
			...
			select_text.set_snaily_text_raw(&"Swap selection - bind__LEFT bind__RIGHT")
		MoveMode.GRID:
	...
	match selection_depth:
		0:
			if map_focused and SInput.check_input(SInput.Inputs.UI_ACCEPT, true):
				...
				map_selector.position = map_sel_origin + (map_selection * 8)
				marker_text.set_snaily_text_raw(&"Place/remove marker - bind__UI_ACCEPT")
				select_text.set_snaily_text_raw(&"Return - bind__UI_BACK")
		1:
			if map_focused:
				if SInput.check_input(SInput.Inputs.UI_ACCEPT, true):
					...
					selection_depth -= 1
					marker_text.set_snaily_text_raw(&"Set markers - bind__UI_ACCEPT")
					select_text.set_snaily_text_raw(&"Swap selection - bind__LEFT bind__RIGHT")


func _set_desc(id:int) -> void:
	desc_name.visible = true
	desc_name.set_snaily_text_raw(GlobalText.get_item_name(id, true))
	var player = int(Statics.current_profile["character"])
	var stacked_shells:StringName = tr(&"")
	match id:
		Item.ItemTypes.PEASHOOTER:
			desc_body.set_snaily_text_raw(&"The first line of defense.  This little gun lets you fire small yet hardy peas at any wayward foe.  They're not the most effective projectile, as they crumble from contact with enemy and surface alike, but they'll do in a pinch.\n\n\n[color=#ffd48c]Hold the SHOOT button to fire")
		Item.ItemTypes.BOOMERANG:
			desc_body.set_snaily_text_raw(&"These little things can pack quite the punch!  Their points and edges are refined to catch wind and foe alike.  When tossed at just the right distance, the turnaround can score some big damage!\n\n\n[color=#ffd48c]Hold the SHOOT button to fire")
		Item.ItemTypes.RAINBOW_WAVE:
			desc_body.set_snaily_text_raw(&"It's said that those who do good in the eyes of Iris are offered a small piece of her power.  Lucky you!! These sharpened shards of solid light can cut through just about any shell, wall, or particularly stubborn slime.\n\n\n[color=#ffd48c]Hold the SHOOT button to fire")
		Item.ItemTypes.DEVASTATOR:
			desc_body.set_snaily_text_raw(&"An old relic of unknown origin, said to empower the bearer with strength and fury to rival even the highest of gods.  At least, I think so; I may have slept through that class.  Regardless, having this on you powers up all of your attacks!  Neat, huh?")
		Item.ItemTypes.HIGH_JUMP:
			if player == Player.Players.BLOBBY:
				desc_body.set_snaily_text_raw(&"Contained inside is a ready-made meal crafted specifically to help make a blob's body stickier.  It's mostly hard candy.\n\n\n[color=#ffd48c]Hold toward a wall or ceiling while airborne to stick to it")
			else:
				desc_body.set_snaily_text_raw(&"This badge is actually a container full of nothing but helium, allowing its wearer a little more air time when they jump!  Mind the fall, though; it doesn't cushion the landing very well.\n\n\n[color=#ffd48c]Hold the JUMP button to jump as high as possible")
		Item.ItemTypes.SHELL_SHIELD:
			if player == Player.Players.BLOBBY:
				desc_body.set_snaily_text_raw(&"Now isn't this a nice find!!  This helmet looks just about sturdy enough to shrug off a hit or two and come out unscathed.  Plus, it's fashionable!!\n\n\n[color=#ffd48c]Press toward the ground to hide under it")
			else:
				desc_body.set_snaily_text_raw(&"With a little polish and protective slime, your shell is now capable of withstanding damage!  From the lightest graze to the heaviest blow, nothing is too much to handle anymore! Just remember to buff out the scratches when you get home.\n\n\n[color=#ffd48c]Press toward the ground to hide in your shell")
		Item.ItemTypes.RAPID_FIRE:
			if player == Player.Players.LEECHY:
				desc_body.set_snaily_text_raw(&"subscreen_desc_backfire")
			else:
				desc_body.set_snaily_text_raw(&"An intriguing little gizmo.  Even just holding it makes you feel more nimble and energetic!  Throwing attacks at anything in your way should be much easier and quicker now.")
		Item.ItemTypes.ICE_SHELL:
			desc_body.set_snaily_text_raw(&"This frosty relic grants its bearer a close attunement with the cold, preventing harm from any icy adversaries and environmental hazards.  It also gives your body a lovely shine!")
		Item.ItemTypes.GRAVITY_SHELL:
			match player:
				Player.Players.UPSIDE: desc_body.set_snaily_text_raw(&"subscreen_desc_magneticFoot")
				Player.Players.LEGGY: desc_body.set_snaily_text_raw(&"subscreen_desc_corkscrewJump")
				Player.Players.BLOBBY: desc_body.set_snaily_text_raw(&"subscreen_desc_angelJump")
				_: desc_body.set_snaily_text_raw(&"This badge is crafted from discarded gravity turtle scutes that still carry some of that innate control over the force.  With a little concentration, wearing this badge allows the wearer to redirect which direction they get pulled.\n\n\n[color=#ffd48c]Hold a direction and press the GRAVITY button to flip gravity")
			if Statics.stack_shells:
				desc_body.set_snaily_text_raw(desc_body.text + stacked_shells)
		Item.ItemTypes.NONE:
			desc_body.set_snaily_text_raw(&"It's you!\nThis is your normal self; how you've always known yourself.  You can take a hit or two, but not much else.  But hey, that just means there's room to grow!")
			if Statics.get_world_flag(Statics.WorldFlags.DEFEATED_BOSS4):
				desc_body.set_snaily_text_raw(&"Despite everything, it's still you.\nYou've come far.  You've changed.  Even through it all, though, it's never a bad idea to remember who you are at your core.  Self love is important, you know!")
		-2:
			#var pkeys = Player.Players.keys()
			desc_name.visible = false
			#desc_body.set_snaily_text_raw(&"subscreen_desc_" + pkeys[player].to_lower())
			match player:
				Player.Players.SNAILY:
					desc_body.set_snaily_text_raw(tr(&"Snaily is just your average slow-going, grass-eating, fun-loving snail.  There isn't much to separate them from the rest of Snail Town, except for one thing: their drive.  Snail Town picked them for this adventure knowing Snaily wouldn't shy away and would get it done, no matter how hard it would be.\n\nSnaily has the strength to climb up and around walls and ceilings, and sports a shell to hide in when things get scary."))
				Player.Players.SLUGGY:
					desc_body.set_snaily_text_raw(tr(&"Slugs are not an uncommon sight around these parts, but none are quite as daring and adventurous as Sluggy!  Sluggy doesn't let the threat of danger scare them away from the adventure of a lifetime, and believes Snail Town's faith is not misplaced with them.\n\nSluggy's lack of a shell makes them take a little more damage, but also move faster and jump higher than a snail."))
				Player.Players.UPSIDE:
					desc_body.set_snaily_text_raw(tr(&"Upside-Down Snail is a quirky one.  They're part gravity snail, but never quite figured out how exactly to get ahold of this power.  As such, they're locked to always falling upward.  They've gotten used to it, though, as have all their friends in town.\n\nIn spite of their strange gravity, Upside can climb walls and floors like any other snail, and their shell is just as good for hiding."))
				Player.Players.LEGGY:
					desc_body.set_snaily_text_raw(tr(&"Leggy hasn't always lived in Snail Town, but it didn't take very long at all for him to be accepted into it.  After all, he's slow and has a shell!  When he's not tracking down missing snails, Leggy likes to relax and write music.\n\nLeggy is slower and can't climb walls, but he can flip his gravity off the bat."))
				Player.Players.BLOBBY:
					desc_body.set_snaily_text_raw(tr(&"Not every blob is out to get you!!  Blobby is one such blob who has made peace with Snail Town, and has set out on this adventure to help its friends however it can.  Even if that means fighting off other blobs.\n\nBlobby can't climb walls, instead leaning into its innate talent for jumping and the unique abilities that come with being a blob."))
				Player.Players.LEECHY:
					desc_body.set_snaily_text_raw(tr(&"Leechy desc"))
		_:
			desc_name.set_snaily_text_raw("")
			desc_body.set_snaily_text_raw("")
